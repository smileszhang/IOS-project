...
let SERVER_URL = "http://10.8.148.37:8000" // change this for your server name!!!
...
//            break
    ...
    override func viewDidLoad() {
        ...
        dsid = 3 // set this and it will update UI
    }
        ...
        let postTask : URLSessionDataTask = self.session.dataTask(with: request,
                  completionHandler:{(data, response, error) in
                    if(error != nil){
                        ...
                        DispatchQueue.main.asyncAfter(deadline: .now() + 0.05, execute: {
                            self.knnLabel.text = labelResponse_KNN as! String
                            self.svmLabel.text = labelResponse_SVC as! String

                        })
                       // self.displayLabelResponse(labelResponse_SVC as! String)
                    }
